...
# ----------
#  TODO 9 - Naïve search vs spatial hashing
# ----------
function run_todo9()
    ...
    for (label, p) in configs
        r_hash  = run_experiment(label, p, kern)
        r_naive = run_experiment_naive(label, p, kern)
...
# ----------
#  TODO 10 - Euler vs Euler-Cromer stability
#  Same initial conditions; only integrator differs.
# ----------
function run_todo10()
    ...
    configs = [
        ("Stable   dt=2e-5 (default)", default_params()),
        ("Moderate dt=5e-5",           default_params(dt=5.0e-5)),
    ...
    for (label, p) in configs
        r_cromer = run_experiment(label, p, kern; integrator=:cromer)
        r_euler  = run_experiment(label, p, kern; integrator=:euler)
